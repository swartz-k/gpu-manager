...
func (nl *nodeLabeler) Run(ctx context.Context) error {
	err := wait.PollImmediate(time.Second, time.Minute, func() (bool, error) {
		node, err := nl.client.Nodes().Get(ctx, nl.hostName, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		...
		_, updateErr := nl.client.Nodes().Update(ctx, node, metav1.UpdateOptions{})
		if updateErr != nil {
